...
def index():
	...
	if 'username' not in login_session:
		return render_template('index.html', chaf=chaf, table=table, house=house, counter=counter, tab=tab, catimg=catimg,flag=1,log=0)
	else:
		if login_session['admin']==0:
			return render_template('index.html', chaf=chaf, table=table, house=house, counter=counter, tab=tab, catimg=catimg,flag=1,log=1)
		else:
			return render_template('indexadmin.html', chaf=chaf, table=table, house=house, counter=counter, tab=tab, catimg=catimg,flag=1,log=2)
...
def productcreate(productcat):
	...
	if login_session['admin'] != 1:
		...
		return redirect('/restaurants')
	if request.method == 'POST':
		newItem = Product(name=request.form['name'], catagory=productcat, id=request.form['id'], size=request.form['size'],
